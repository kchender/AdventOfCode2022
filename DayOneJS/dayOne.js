...
// ouput: total Calories that elf carrying the most calories is carrying (highest sum of numbers separate by two newline characters)
  ...
  // sum of three Max values value from totals array - Math.max is O(n), so if I run it three times it O (3n) which isn't bad... but looping through can reduce that\
  // mergesort = O (n log n) Timesort = (O n)
